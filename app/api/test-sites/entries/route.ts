...
export async function GET(request: NextRequest) {
  try {
    console.log('=== GET TEST ENTRIES API CALLED ===')
    ...
    // Если не тестировщик, показываем только свои записи
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to fetch test entries' }, { status: 500 })
    }

    return NextResponse.json({ entries: entries || [] })
  } catch (error) {
    console.error('Error in GET /api/test-sites/entries:', error)
...
export async function POST(request: NextRequest) {
  try {
    console.log('=== POST TEST ENTRIES API CALLED ===')
    ...
    const validatedData = testEntrySchema.parse(body)
    ...
    if (insertError) {
      ...
      return NextResponse.json({ error: 'Failed to create test entry' }, { status: 500 })
    }

    // Логируем создание тестовой записи
    await logActivity(
      currentUser.id,
      'test_entry_created',
      {
    ...
    console.error('Error in POST /api/test-sites/entries:', error)
